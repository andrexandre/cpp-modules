...
void Bureaucrat::signForm(AForm &form) const
{
	...
	{
		co << MAGENTA << this->_name << " couldn't sign " << form.getName() << END << nl;
		co << "Error: " << e.what() << nl;
	}
}
...
void Bureaucrat::executeForm(AForm const &form) const
{
	...
	{
		form.execute(*this);
		co << GREEN << this->_name << " executed " << form.getName() << END << nl;
	}
	catch(const std::exception &e)
	{
		co << MAGENTA << this->_name << " couldn't execute " << form.getName() << END << nl;
		co << "Error: " << e.what() << nl;
	}
}
